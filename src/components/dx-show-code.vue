<template>
  <div class="plane">
    ...
    <div class="show-btn" @click="showCode = !showCode">
      {{ statusText }}代码
    </div>
...
<script>
import formatCode from "@/utils/parseCode";
export default {
  name: "DxShowCode",
  data() {
    return {
  ...
  computed: {
    ...
    formatScript: function () {
      return (
        this.scriptString &&
        formatCode("formatSptAndStyCode", this.scriptString)
      );
    },
    formatStyle: function () {
      return (
        this.styleString && formatCode("formatSptAndStyCode", this.styleString)
      );
    },
    statusText: function () {
      if (this.showCode) {
